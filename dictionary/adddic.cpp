...
#include "about.h"
using namespace std;
bool found;//результат знаходження слова
...
{
    ofstream file("dictionary.txt",ios_base::app);//дописую в кінець файлу
    file  << endl << engWord <<' ';
    file << uaWord ;
    file.close();
...
void createNode(char uaWord[], char engWord[], node **tree)
{
    ...
    if ((*tree)->hashEn > hashEn)//(порівняння)
        createNode(uaWord, engWord, &(*tree)->right); // to right якщо >0
    else
        createNode(uaWord, engWord, &(*tree)->left);//  to left if <0
}

// повертає found = true якщо знайдено слово в словнику і false якщо ні (для перевірки під час додавання new word)
void find(node *tree, bool typeDic, char findWord[])//Пошук слова
{
    start_chrono();//start лічильник часу
    size_t hashT;
...
void Load(node **tree)//завантаження з файлу слів
{
    int max_lenght=1024;
    char buffer[50];
    ...
    if (!file.is_open()){ // якщо файл не відкритий
           // повідомити про це
           file.open("dictionary.txt", ios_base::out| ios_base::trunc);//буде створений
            error_open();
    ...
    while (!file.eof())
    {
        file >> buffer;
        //file >> engWordT;
        char ch; file.get(ch);//Считываем символ из файла
...
void menu(){
    bool typeDic = true;//за умовчанням English
    ...
    while (true)
    {
       ...
       typeDic = menuRun(buffer, buffer2,typeDic);//обробка запиту меню
    }
}
...
bool menuRun(char buffer[], char buffer2[], bool typeDic){
    ...
    do{
        ...
        cin >> c;
        menu = c - '0';
         if(!(isdigit(c)) || menu >4){//перевірка на введення команди
